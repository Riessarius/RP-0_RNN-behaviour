...
class RNNModel(nn.Module):
    def __init__(self, rnn_type: str,
        ...
        if self._num_embeddings is not None and self._embedding_dims is not None:
            assert len(self._num_embeddings) == len(self._embedding_dims), f"The list of number of embedding and embedding dimension must have the same length."
            embeddings = []
    ...
    def forward(self, x: Union[torch.Tensor, Tuple[torch.Tensor]], /) -> torch.tensor:
        if self._embeddings is not None:
            ...
            assert info.shape[1] == len(self._embeddings), f"The number of embeddings must be the same as the number of embedding layers."
